...
class TableBuilder
  ...
  def build_table(&block)
    ...
    concat("<form method=\"get\">") if @table_options[:make_form]
    ...
    concat(make_tag(:table, @table_options[:table_html]))
    concat(make_tag(:thead))

    render_table_header
    render_table_filters if @table_options[:filter]
    ...
    concat("</tbody></table>")
    concat('</form>') if @table_options[:make_form]
    @val.join("\n")
  end
